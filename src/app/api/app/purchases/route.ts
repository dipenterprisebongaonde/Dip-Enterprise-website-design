...
import { applyPurchaseToStock } from "@/lib/stock";
import {
  parseInvoiceRequest,
  proofErrorMessage,
  savePaymentProof,
...
export async function GET() {
  ...
  const purchases = await prisma.purchase.findMany({
    where,
    include: { vendor: true, branch: true, lines: true },
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (parsed.proofFile) {
      ...
      try {
        proof = await savePaymentProof(parsed.proofFile, "purchase");
      } catch (uploadError) {
    ...
    try {
      invoiceNo = await resolveInvoiceNumber("purchase", data.invoiceNo, {
    ...
    if (settleFromAdvance) {
      try {
        await consumePartyAdvance({
          ...
          amount: paidAmount,
          paidAt: new Date(data.paidAt || data.invoiceDate),
          note: `Settled on ${purchase.invoiceNo}`,
        });
      } catch (advanceError) {
        const code = advanceError instanceof Error ? advanceError.message : "";
