...
import { recordCustomerPartyPayment } from "@/lib/party-payments";
...
export async function POST(
  ...
  try {
    ...
    if (raw.proofFile) {
      try {
        proof = await savePaymentProof(raw.proofFile, `customer-${id.slice(-6)}`);
      } catch (uploadError) {
    ...
    const result = await recordCustomerPartyPayment({
      customerId: id,
      amount: data.amount,
