...
import { applySaleToStock, reverseSaleFromStock } from "@/lib/stock";
import {
  parseInvoiceRequest,
  proofErrorMessage,
  savePaymentProof,
...
export async function PUT(
  ...
  try {
    ...
    if (parsed.proofFile) {
      ...
      try {
        proof = await savePaymentProof(parsed.proofFile, `sale-${id.slice(-6)}`);
      } catch (uploadError) {
    ...
    try {
      for (const line of previousLines) {
        await reverseSaleFromStock({
          ...
          quantity: line.quantity,
          note: `Sale edit reverse ${existing.invoiceNo}`,
        });
      }
      for (const line of summary.lines) {
        await applySaleToStock({
          ...
          productName: line.item,
          quantity: line.quantity,
          note: `Sale edit apply ${data.invoiceNo}`,
        });
      }
    } catch (stockError) {
      try {
        for (const line of previousLines) {
          await applySaleToStock({
            ...
            productName: line.item,
            quantity: line.quantity,
            note: `Sale edit rollback ${existing.invoiceNo}`,
          });
        }
      } catch {
      ...
      if (code === "PRODUCT_NOT_FOUND") {
        return NextResponse.json(
          { error: "Product not found in inventory. Add the product first." },
          { status: 400 }
        );
      }
      if (code === "INSUFFICIENT_STOCK") {
        return NextResponse.json(
          { error: "Not enough stock for this updated sale." },
          { status: 400 }
        );
      }
    ...
    try {
      await assertInvoiceNoAvailable("sale", data.invoiceNo, { excludeId: id });
    } catch (error) {
    ...
    if (paidAmount > 0 && data.paidAt) {
      const latest = await prisma.salePayment.findFirst({
        where: { saleId: id },
        orderBy: { paidAt: "desc" },
      });
    ...
    return NextResponse.json({ error: "Could not update sale invoice" }, { status: 400 });
  }
}
