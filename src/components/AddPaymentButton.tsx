...
export function AddPaymentButton({
  ...
  function openForm(nextMode: PayMode) {
    ...
    if (nextMode === "SETTLEMENT") {
      setAmount(settleMax > 0 ? amountFieldValue(settleMax) : "");
    } else {
    ...
    return (
      <div className="payment-meta">
        <span className="muted">
          {paidAmount > 0
            ? `Fully paid · ₹${paidAmount.toLocaleString("en-IN")}`
            : "Fully paid"}
        </span>
  ...
  return (
    <div className="payment-actions">
      <div className="payment-meta">
        <span className="muted">
          {paidAmount > 0 ? `Paid ₹${paidAmount.toLocaleString("en-IN")}` : "Paid"}
        </span>
        {Number(advanceBalance) > 0 ? (
          <span className="payment-chip">
            Advance ₹{Number(advanceBalance).toLocaleString("en-IN")}
          </span>
        ) : null}
        {lastPaidLabel ? <span className="muted">Last paid {lastPaidLabel}</span> : null}
        {proofView}
